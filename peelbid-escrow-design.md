...
- No token
- No auction logic in the escrow: it holds money, nothing else. (Auctions later got a contract of their own; see §25.)
...
**Who decides in v1:** the operator. Stated plainly in the terms, with no pretence of decentralisation.
...
- `timestamp`: expected. Every window is measured in days; a ±15s miner drift is irrelevant. Slither also files unrelated comparisons (`index >= length`, `fee > 0`) under this heading, a tool quirk.
...
Email is the login, confirmed by a one-time code sent to it: entering an existing address returns that account rather than erroring. X handles are unverified by design; the cost of gaming that is 10 Peels and the friction of verification would cost more.
...
**4. Proof checking.**
Compare a monthly proof against the application photo: is the sticker still there, on the right panel, and is this a fresh capture rather than a reused one? Strategically the most valuable (verification is the gap the whole category has left open, see the opening) and the hardest to get right.
...
**The events carry what they should.** `BidPlaced` records the total, the run length *and the monthly rate the contract derived*, so ranking is enforced on-chain, not a convention the site upholds. `AuctionEnded` was called by the bidder, not the owner, which is `endAuction` being permissionless in practice rather than in principle.
...
**Privy didn't know Arc existed.** *"Chain ID 5042002 is not supported"* reads as a wallet fault and is a missing four-line definition.
...
**External wallets don't follow a chainId.** MetaMask stayed on whatever network it was last on and priced an Arc transaction in BNB. Arc is in no wallet's default list either, so switching fails with "unrecognized chain", which is a request to add it, not an error. Asking somebody to find Arc in a dropdown is asking them to give up.
...
payment window if the winner never pays. All three calls are permissionless.
...
the earlier contract tests, with nothing unaccounted for.
